...
import { $env, getNativesDir, logger } from "@reagent/ra-utils";
...
const userDataDir =
	process.platform === "win32"
		? path.join(Bun.env.LOCALAPPDATA || path.join(os.homedir(), "AppData", "Local"), "reagent")
...
function loadNative(): NativeBindings {
	...
	if (isCompiledBinary) {
		...
		helpMessage =
			"If installed via npm/bun, try reinstalling: bun install @reagent/ra-natives\n" +
