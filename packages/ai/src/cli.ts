...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		console.log(`Usage: bunx @reagent/ra-ai <command> [provider]
...
Examples:
  bunx @reagent/ra-ai login              # interactive provider selection
  bunx @reagent/ra-ai login anthropic    # login to specific provider
  bunx @reagent/ra-ai logout anthropic   # logout from specific provider
  bunx @reagent/ra-ai status             # show logged-in providers
  bunx @reagent/ra-ai list               # list providers
`);
		return;
	}
	...
	if (command === "status") {
		...
		try {
			...
			if (providers.length === 0) {
				...
				console.log(`Use 'bunx @reagent/ra-ai login' to authenticate.`);
			} else {
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some(p => p.id === provider)) {
			...
			console.error(`Use 'bunx @reagent/ra-ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'bunx @reagent/ra-ai --help' for usage`);
	process.exit(1);
}
