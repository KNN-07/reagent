 ...
 */
import { getEnvApiKey } from "@reagent/ra-ai";
...
export async function searchKimi(params: KimiSearchParams): Promise<SearchResponse> {
	...
	if (!apiKey) {
		throw new Error(
			"Kimi search credentials not found. Set MOONSHOT_SEARCH_API_KEY, KIMI_SEARCH_API_KEY, MOONSHOT_API_KEY, or login with 'reagent /login moonshot'.",
		);
	}
