...
import { $env, getAgentDbPath, readSseJson } from "@reagent/ra-utils";
...
export async function searchCodex(params: CodexSearchParams): Promise<SearchResponse> {
	...
	if (!auth) {
		throw new Error(
			"No Codex OAuth credentials found. Login with 'reagent /login openai-codex' to enable Codex web search.",
		);
	}
