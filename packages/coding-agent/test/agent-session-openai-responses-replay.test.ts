...
import type { AssistantMessage, Message, ProviderPayload, ProviderSessionState, Usage } from "@reagent/ra-ai/types";
import { createOpenAIResponsesHistoryPayload } from "@reagent/ra-ai/utils";
import type { AgentSession } from "@reagent/ra-coding-agent/session/agent-session";
import type { AuthStorage } from "@reagent/ra-coding-agent/session/auth-storage";
import {
	type SessionEntry,
	SessionManager,
	type SessionMessageEntry,
} from "@reagent/ra-coding-agent/session/session-manager";
...
async function createSessionHarness(
	...
	const [{ createAgentSession }, { Settings }, { AuthStorage }] = await Promise.all([
		import("@reagent/ra-coding-agent/sdk"),
		import("@reagent/ra-coding-agent/config/settings"),
		import("@reagent/ra-coding-agent/session/auth-storage"),
	]);
