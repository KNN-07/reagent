...
2. **Local shared gateway** (default path)
   - Uses a single shared process coordinated under `~/.reagent/agent/python-gateway`.
...
6. Load extension modules from:
   - user: `~/.reagent/agent/modules/*.py`
   - project: `<cwd>/.reagent/modules/*.py` (overrides same-name user module)
...
2. Managed venv at `~/.reagent/python-env`
