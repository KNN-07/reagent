...
This document deep-dives the addon loading/validation layer in `@reagent/ra-natives`: how `native.ts` decides which `.node` file to load, when embedded payload extraction runs, and how startup failures are reported.
...
- reinstall hint (`bun install @reagent/ra-natives`),
