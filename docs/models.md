...
1. Load built-in providers/models from `@reagent/ra-ai`.
...
One notable legacy path remains: web-search Anthropic auth resolution still reads `~/.reagent/agent/models.json` directly in `src/web/search/auth.ts`.
