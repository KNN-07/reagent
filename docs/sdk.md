...
The SDK is the in-process integration surface for `@reagent/ra-coding-agent`.
...
```bash
bun add @reagent/ra-coding-agent
```

## Entry points

`@reagent/ra-coding-agent` exports the SDK APIs from the package root (and also via `@reagent/ra-coding-agent/sdk`).
...
```ts
import { createAgentSession } from "@reagent/ra-coding-agent";
...
- `cwd`: `getProjectDir()`
- `agentDir`: `~/.reagent/agent` (via `getAgentDir()`)
...
```ts
import { createAgentSession, SessionManager } from "@reagent/ra-coding-agent";
...
```ts
import { SessionManager } from "@reagent/ra-coding-agent";
