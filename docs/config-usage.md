...
`CONFIG_DIR_NAME` is `.reagent` (`packages/utils/src/dirs.ts`).
...
Walks parent directories upward and returns the **nearest existing directory per source base** (`.reagent`, `.claude`, `.codex`, `.gemini`), then sorts results by source priority.
...
1. Global settings: `~/.reagent/agent/config.yml`
...
1. Migrate from `~/.reagent/agent/settings.json` (renamed to `.bak` on success)
...
## 6) Native `.reagent` provider behavior (`src/discovery/builtin.ts`)
...
For `SYSTEM.md` and `AGENTS.md`, native provider uses nearest-ancestor project `.reagent` directory search (walk-up) but still requires the `.reagent` dir to be non-empty.
...
- Extension manifest compatibility: loader accepts both `package.json.reagent` and `package.json.pi` manifest sections.
