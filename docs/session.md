...
```text
~/.reagent/agent/sessions/--<cwd-encoded>--/<timestamp>_<sessionId>.jsonl
```
...
```text
~/.reagent/agent/blobs/<sha256>
```

Terminal breadcrumb files are written under:

```text
~/.reagent/agent/terminal-sessions/<terminal-id>
```
...
- `listAll()` -> sessions across all project scopes under `~/.reagent/agent/sessions`
