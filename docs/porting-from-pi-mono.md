...
- Examples (adjust to match the actual packages you are porting):
  - `@mariozechner/pi-coding-agent` → `@reagent/ra-coding-agent`
  - `@mariozechner/pi-agent-core` → `@reagent/ra-agent-core`
  - `@mariozechner/pi-tui` → `@reagent/ra-tui`
  - `@mariozechner/pi-ai` → `@reagent/ra-ai`
...
- In coding-agent, never use `console.log`/`console.warn`/`console.error`; use `logger` from `@reagent/ra-utils`.
...
| Upstream                                           | Our Fork                                | Reason                                  |
| -------------------------------------------------- | --------------------------------------- | --------------------------------------- |
| `clipboard.ts` + `clipboard-image.ts` (tool files) | `@reagent/ra-natives` clipboard module | Merged into N-API native implementation |
...
| `pkg.pi` manifest field       | `pkg.reagent` (prefer our namespace) |
...
- `clipboard-image.ts` — clipboard is in `@reagent/ra-natives` N-API module
