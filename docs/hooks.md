...
```ts
import type { HookAPI } from "@reagent/ra-coding-agent/hooks";
...
Discovery providers for `hookCapability` still model pre/post shell-style hook files (for example `.claude/hooks/pre/*`, `.reagent/.../hooks/pre/*`).
