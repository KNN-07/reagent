...
This document explains how token/tool streaming is normalized in `@reagent/ra-ai`, then propagated through `@reagent/ra-agent-core` and `coding-agent` session events.
...
## Unified stream contract in `@reagent/ra-ai`
