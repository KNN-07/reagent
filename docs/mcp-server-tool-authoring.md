...
```text
Config sources (.reagent/.claude/.cursor/.vscode/mcp.json, mcp.json, etc.)
...
In practice MCP servers also come from higher-priority providers (for example native `.reagent/...` and tool-specific config dirs). Authoring guidance:

- Prefer `.reagent/mcp.json` (project) or `~/.reagent/mcp.json` (user) for explicit control.
